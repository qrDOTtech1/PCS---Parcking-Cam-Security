...
MODES = {
    "standard":  {"label":"Standard",          "color":"#3b82f6", "max_cameras":3,   "max_blacklist":20,   "days":30,  "features":[]},
    "pro":       {"label":"Pro",               "color":"#8b5cf6", "max_cameras":10,  "max_blacklist":100,  "days":365, "features":["vehicle_filters","priority_alerts","anpr_history","multi_notify","gps_tracking"]},
    "emergency": {"label":"Centre d'Urgence",  "color":"#ef4444", "max_cameras":50,  "max_blacklist":500,  "days":365, "features":["vehicle_filters","type_color_only","priority_alerts","anpr_history","multi_notify","gps_tracking"]},
    "enterprise":{"label":"Enterprise",        "color":"#f59e0b", "max_cameras":100, "max_blacklist":-1,   "days":365, "features":[f[0] for f in ALL_FEATURES]},
    "custom":    {"label":"Custom",            "color":"#6b7280", "max_cameras":None,"max_blacklist":None, "days":None,"features":[]},
}
...
class AdminPCSApp:
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill=tk.X)
        tk.Label(hdr, text="ParkingCamSecurity — Admin Panel",
                 font=("Arial", 15, "bold"), fg="#3b82f6", bg="#1e293b").pack(side=tk.LEFT, padx=16)
        self.global_status = tk.Label(hdr, text="", font=("Arial", 9),
                                       fg="#fbbf24", bg="#1e293b")
        self.global_status.pack(side=tk.LEFT, padx=8)
        tk.Button(hdr, text="Manage Servers", bg="#7c3aed", fg="white",
                  relief=tk.FLAT, command=self._servers_dialog).pack(side=tk.RIGHT, padx=6)
        tk.Button(hdr, text="Refresh All", bg="#059669", fg="white",
                  relief=tk.FLAT, command=self._refresh_all).pack(side=tk.RIGHT, padx=4)

        # ── Create account strip (top, cross-server) ──
        create_strip = tk.Frame(self.root, bg="#1e293b", pady=8, padx=12)
        create_strip.pack(fill=tk.X)
        tk.Label(create_strip, text="New Client:", font=("Arial", 10, "bold"),
                 fg="#f8fafc", bg="#1e293b").pack(side=tk.LEFT, padx=(0,8))
        tk.Label(create_strip, text="Username", fg="#94a3b8", bg="#1e293b").pack(side=tk.LEFT)
        self.e_user = ttk.Entry(create_strip, width=18)
        self.e_user.pack(side=tk.LEFT, padx=4)
        tk.Label(create_strip, text="Password", fg="#94a3b8", bg="#1e293b").pack(side=tk.LEFT)
        ...
        for srv in self.config["servers"]:
            ...
            tk.Checkbutton(create_strip, text=srv["name"], variable=var,
                           fg=srv.get("color","#3b82f6"), bg="#1e293b",
                           selectcolor="#0f172a", activebackground="#1e293b",
                           font=("Arial", 9, "bold")).pack(side=tk.LEFT, padx=4)

        tk.Button(create_strip, text="Create Account(s)", bg="#2563eb", fg="white",
                  font=("Arial", 10, "bold"), relief=tk.FLAT,
                  command=self._create_user).pack(side=tk.LEFT, padx=10)
    ...
    def _build_server_tab(self, srv_id):
        ...
        sbar.pack(fill=tk.X)
        status_lbl = tk.Label(sbar, text=f"URL: {srv['base_url']}",
                               fg="#64748b", bg="#1e293b", font=("Arial", 8))
        status_lbl.pack(side=tk.LEFT)
        conn_lbl = tk.Label(sbar, text="●", fg="#fbbf24", bg="#1e293b", font=("Arial", 12))
        conn_lbl.pack(side=tk.LEFT, padx=6)
        info["conn_lbl"] = conn_lbl
        info["status_lbl"] = status_lbl

        tk.Button(sbar, text="Test", bg="#475569", fg="white", relief=tk.FLAT,
                  command=lambda s=srv_id: self._test_connection(s)
                  ).pack(side=tk.RIGHT, padx=4)
        tk.Button(sbar, text="Refresh", bg="#059669", fg="white", relief=tk.FLAT,
        ...
        specs = [("id","ID",32),("username","Username",115),("mode","Mode",90),
                 ("cameras","Cam",40),("rules","Rules",45),("features","Features",190),
        ...
        tk.Label(r1, text="+ Days:", fg="#94a3b8", bg="#0f172a", font=("Arial",8)
                 ).pack(side=tk.LEFT, padx=(0,3))
        for days, label, color in [(30,"+30d","#16a34a"),(90,"+90d","#15803d"),(365,"+1yr","#14532d")]:
            tk.Button(r1, text=label, bg=color, fg="white", font=("Arial",8,"bold"),
                      relief=tk.FLAT,
                      command=lambda d=days, s=srv_id: self._add_sub(s, d)
                      ).pack(side=tk.LEFT, padx=2, ipadx=5, ipady=1)
        tk.Button(r1, text="Custom…", bg="#475569", fg="white", font=("Arial",8),
                  relief=tk.FLAT, command=lambda s=srv_id: self._custom_days_dlg(s)
                  ).pack(side=tk.LEFT, padx=4, ipadx=5, ipady=1)

        # Action row 2 — modes
        r2 = tk.Frame(frame, bg="#0f172a")
        r2.pack(fill=tk.X, padx=8, pady=1)
        tk.Label(r2, text="Mode:", fg="#94a3b8", bg="#0f172a", font=("Arial",8)
                 ).pack(side=tk.LEFT, padx=(0,3))
        for key, mi in MODES.items():
            tk.Button(r2, text=mi["label"], bg=mi["color"], fg="white",
        ...
        for label, color, cmd in actions:
            tk.Button(r3, text=label, bg=color, fg="white", font=("Arial",8,"bold"),
                      relief=tk.FLAT, command=cmd
                      ).pack(side=tk.LEFT, padx=2, ipadx=5, ipady=1)
        tk.Button(r3, text="Delete Client", bg="#dc2626", fg="white",
                  font=("Arial",8,"bold"), relief=tk.FLAT,
        ...
        if other_servers:
            tk.Button(r3, text="Copy to →", bg="#1e3a5f", fg="white",
                      font=("Arial",8,"bold"), relief=tk.FLAT,
    ...
    def _refresh_all(self):
        ...
        for srv_id in self._server_tabs:
            threading.Thread(target=self._load_users, args=(srv_id, True),
                             daemon=True).start()
    ...
    def _test_connection(self, srv_id):
        ...
        try:
            ...
            if res.status_code in [200, 401]:
                messagebox.showinfo("OK", f"{srv['name']} responding — HTTP {res.status_code}")
            else:
    ...
    def _send_sub(self, srv_id, username, payload):
        ...
        try:
            res = _req("POST", f"{srv['base_url']}/api/admin/users/{username}/subscription",
                       srv["master_key"], json=payload)
            data = res.json()
            if res.status_code == 200:
    ...
    def _create_user(self):
        ...
        targets = [srv for srv in self.config["servers"]
        ...
        for srv in targets:
            try:
                res = _req("POST", f"{srv['base_url']}/api/admin/users",
                           srv["master_key"], json={"username": user, "password": pwd})
                data = res.json()
                status = "✓" if res.status_code == 201 else f"✗ {data.get('message','err')}"
    ...
    def _delete_user(self, srv_id):
        ...
        if not username:
            return
        srv = self._server_tabs[srv_id]["srv"]
        if not messagebox.askyesno("Confirm", f"Delete '{username}' from {srv['name']}?"):
            return
        try:
            res = _req("DELETE", f"{srv['base_url']}/api/admin/users/{username}",
                       srv["master_key"])
            data = res.json()
            if res.status_code == 200:
    ...
    def _apply_mode(self, srv_id, mode_key):
        ...
        if mode_key == "custom":
            ...
            return
        mi = MODES[mode_key]
        msg = (f"Apply '{mi['label']}' to '{username}' on {self._server_tabs[srv_id]['srv']['name']}?\n\n"
        ...
        if not messagebox.askyesno("Confirm", msg):
            return
        self._send_sub(srv_id, username, {
            "mode": mode_key, "max_cameras": mi["max_cameras"],
            "max_blacklist": mi["max_blacklist"], "days": mi["days"],
    ...
    def _cam_limit_dlg(self, srv_id):
        ...
        if not username:
            return
        dlg = self._mini_dialog(f"Camera Limit — {username}", "Max cameras:",
                                str(u.get("max_cameras",3)), allow_neg=False)
        def apply(val):
            try:
    ...
    def _rule_limit_dlg(self, srv_id):
        ...
        if not username:
            return
        dlg = self._mini_dialog(f"Rule Limit — {username}", "Max rules (−1=∞):",
                                str(u.get("max_blacklist",50)), allow_neg=True)
        def apply(val):
            try:
    ...
    def _mini_dialog(self, title, label, default, allow_neg=False):
        ...
        entry.pack(pady=4)
        btn = tk.Button(dlg, text="Set", bg="#2563eb", fg="white",
                        font=("Arial",10,"bold"))
        btn.pack(pady=8)
    ...
    def _features_dlg(self, srv_id):
        ...
        dlg.transient(self.root)
        dlg.grab_set()
        tk.Label(dlg, text=f"Feature flags — {username}",
                 fg="#f8fafc", bg="#1e293b", font=("Arial",11,"bold")).pack(pady=(12,6))
        vars_ = {}
        ...
        for label, keys in [("All", [k for k,_ in ALL_FEATURES]), ("None", [])]:
            def toggle(kl=keys):
                for k,v in vars_.items(): v.set(k in kl)
            tk.Button(pr, text=label, bg="#475569", fg="white", relief=tk.FLAT,
                      command=toggle).pack(side=tk.LEFT, padx=4)
        def apply():
    ...
    def _notes_dlg(self, srv_id):
        ...
        dlg.transient(self.root)
        dlg.grab_set()
        tk.Label(dlg, text="Internal notes (not visible to client):",
                 fg="#f8fafc", bg="#1e293b").pack(pady=(12,4))
        ...
        tk.Button(dlg, text="Save", bg="#4338ca", fg="white",
                  font=("Arial",10,"bold"), command=save).pack(pady=8)
    ...
    def _details_dlg(self, srv_id):
        ...
        tk.Label(dlg, text=f"Server: {srv['name']}  |  Mode: {(u.get('subscription_mode') or 'standard').capitalize()}",
                 fg="#3b82f6", bg="#1e293b", font=("Arial",9,"bold")).pack()
        ...
        for lbl, val in fields:
            ...
            tk.Label(row, text=f"{lbl}:", fg="#94a3b8", bg="#0f172a",
                     width=14, anchor=tk.W).pack(side=tk.LEFT)
            tk.Label(row, text=val, fg="#e2e8f0", bg="#0f172a",
                     font=("Arial",9,"bold")).pack(side=tk.LEFT)
        feats = u.get("features",[])
        tk.Label(dlg, text="Features:", fg="#94a3b8", bg="#1e293b").pack(anchor=tk.W, padx=16)
        ft = scrolledtext.ScrolledText(dlg, width=50, height=4, bg="#0f172a",
                                        fg="#22c55e", state=tk.NORMAL)
        ft.insert("1.0", "\n".join(feats) if feats else "(none)")
        ft.config(state=tk.DISABLED)
        ft.pack(padx=16)
        notes = u.get("admin_notes","") or ""
        if notes:
            tk.Label(dlg, text="Notes:", fg="#94a3b8", bg="#1e293b").pack(anchor=tk.W, padx=16)
            nt = scrolledtext.ScrolledText(dlg, width=50, height=3, bg="#0f172a",
                                            fg="#fbbf24", state=tk.NORMAL)
            nt.insert("1.0", notes)
    ...
    def _copy_user_dlg(self, src_srv_id):
        ...
        tk.Label(dlg, text=f"Copy account '{username}' to:",
                 fg="#f8fafc", bg="#1e293b", font=("Arial",11,"bold")).pack(pady=(14,8))

        target_var = tk.StringVar(value=other[0]["id"])
        for srv in other:
            tk.Radiobutton(dlg, text=srv["name"], variable=target_var, value=srv["id"],
                           fg=srv.get("color","white"), bg="#1e293b",
                           selectcolor="#0f172a", activebackground="#1e293b",
                           font=("Arial",10,"bold")).pack(padx=20, anchor=tk.W)

        tk.Label(dlg, text="New password (leave blank = keep same):",
                 fg="#94a3b8", bg="#1e293b", font=("Arial",9)).pack(pady=(10,3))
        e_pwd = ttk.Entry(dlg, width=28, show="*")
        e_pwd.pack()

        tk.Label(dlg, text="Also copy subscription/features/mode?",
                 fg="#94a3b8", bg="#1e293b", font=("Arial",9)).pack(pady=(6,2))
        copy_sub_var = tk.BooleanVar(value=True)
        tk.Checkbutton(dlg, text="Yes, copy full plan", variable=copy_sub_var,
                       fg="#e2e8f0", bg="#1e293b", selectcolor="#0f172a",
                       activebackground="#1e293b").pack()
        ...
        def apply():
            ...
            try:
                res = _req("POST", f"{dst_srv['base_url']}/api/admin/users",
                           dst_srv["master_key"],
                ...
                if res.status_code == 201:
                    ...
                    if copy_sub_var.get():
                        ...
                        res2 = _req("POST",
                            ...
                            dst_srv["master_key"], json=payload)
        ...
        tk.Button(dlg, text="Copy Account", bg="#1e3a5f", fg="white",
                  font=("Arial",10,"bold"), command=apply).pack(pady=12)

    # ── Full custom dialog ────────────────────────────────────────────────────

    def _full_custom_dlg(self, srv_id, username):
        u = next((x for x in self._users_cache.get(srv_id,[]) if x["username"]==username), {})
        srv = self._server_tabs[srv_id]["srv"]
        ...
        tk.Label(dlg, text=f"Custom Plan — {username}",
                 fg="#f8fafc", bg="#1e293b", font=("Arial",13,"bold")).pack(pady=(12,2))
        tk.Label(dlg, text=f"Server: {srv['name']}",
                 fg=srv.get("color","#3b82f6"), bg="#1e293b", font=("Arial",9,"bold")).pack()
        ...
        def section(txt):
            ...
            tk.Label(f, text=txt, fg="#3b82f6", bg="#0f172a",
                     font=("Arial",10,"bold")).pack(anchor=tk.W)

        def field(lbl, default):
            row = tk.Frame(sf, bg="#1e293b")
            row.pack(fill=tk.X, padx=8, pady=2)
            tk.Label(row, text=lbl, fg="#cbd5e1", bg="#1e293b",
        ...
        tk.Label(sf, text="  −1 = unlimited rules", fg="#475569", bg="#1e293b",
                 font=("Arial",8)).pack(anchor=tk.W, padx=32)
        ...
        for key, desc in ALL_FEATURES:
            ...
            tk.Checkbutton(row, text=key, variable=v, fg="#e2e8f0", bg="#1e293b",
                           selectcolor="#0f172a", activebackground="#1e293b",
                           font=("Courier",9,"bold"), width=24, anchor=tk.W).pack(side=tk.LEFT)
            tk.Label(row, text=desc, fg="#94a3b8", bg="#1e293b",
                     font=("Arial",8)).pack(side=tk.LEFT)

        pr = tk.Frame(sf, bg="#1e293b")
        pr.pack(fill=tk.X, padx=16, pady=(4,2))
        tk.Label(pr, text="Preset:", fg="#94a3b8", bg="#1e293b", font=("Arial",8)
                 ).pack(side=tk.LEFT, padx=(0,4))
        for pk, pi in MODES.items():
            if pk == "custom": continue
            def sfn(kl=pi["features"]):
                for k,v in fvars.items(): v.set(k in kl)
            tk.Button(pr, text=pi["label"], bg=pi["color"], fg="white",
                      relief=tk.FLAT, font=("Arial",8),
                      command=sfn).pack(side=tk.LEFT, padx=2, ipadx=3, ipady=1)

        section("Internal Notes")
        nf = tk.Frame(sf, bg="#1e293b")
        nf.pack(fill=tk.X, padx=16, pady=4)
        txt_notes = scrolledtext.ScrolledText(nf, width=55, height=3, wrap=tk.WORD,
                                               bg="#0f172a", fg="#e2e8f0")
        ...
        tk.Button(dlg, text="Apply Custom Plan", bg="#2563eb", fg="white",
    ...
    def _servers_dialog(self):
        ...
        tk.Label(dlg, text="Server Configurations",
                 fg="#f8fafc", bg="#1e293b", font=("Arial",12,"bold")).pack(pady=(12,6))
        tk.Label(dlg, text="Each server requires its own URL and Master Key.\n"
                 "Config saved to admin_config.json next to this file.",
                 fg="#94a3b8", bg="#1e293b", font=("Arial",8)).pack()
        ...
        def refresh_list():
            ...
            for i, srv in enumerate(self.config["servers"]):
                ...
                tk.Label(row, text=srv["name"], fg=srv.get("color","white"),
                         bg="#0f172a", font=("Arial",10,"bold"), width=14,
                         anchor=tk.W).pack(side=tk.LEFT)
                tk.Label(row, text=srv["base_url"], fg="#64748b", bg="#0f172a",
                         font=("Arial",8)).pack(side=tk.LEFT, padx=6)
                tk.Button(row, text="Edit", bg="#475569", fg="white", relief=tk.FLAT,
                          command=lambda idx=i: edit_server(idx)
                          ).pack(side=tk.RIGHT, padx=3)
                if len(self.config["servers"]) > 1:
                    tk.Button(row, text="Remove", bg="#dc2626", fg="white", relief=tk.FLAT,
        ...
        def edit_server(idx=None):
            srv = self.config["servers"][idx] if idx is not None else {
                "id": f"server_{len(self.config['servers'])+1}",
                "name": "New Server", "base_url": "https://",
                "master_key": "", "color": "#3b82f6"
            }
            ...
            for label, key, default, show in [
                ...
                tk.Label(edlg, text=label+":", fg="#f8fafc", bg="#1e293b").pack(pady=(8,2))
            ...
            tk.Button(edlg, text="Save", bg="#2563eb", fg="white",
                      font=("Arial",10,"bold"), command=save_server).pack(pady=12)

        tk.Button(list_frame, text="+ Add New Server", bg="#2563eb", fg="white",
                  font=("Arial",10,"bold"), relief=tk.FLAT,
                  command=lambda: edit_server(None)).pack(fill=tk.X, pady=(8,0))
